local panelName = "durabilitySetadv"
  local ui = setupUI([[
Panel
  height: 19
...
macro(10,function()
  ...
  local slot1 = SlotLeft
  local item1 = getInventoryItem(slot1)
  local slot2 = SlotHead
  local item2 = getInventoryItem(slot2)
  local slot3 = SlotBody
  local item3 = getInventoryItem(slot3)
  local slot4 = SlotLeg
  local item4 = getInventoryItem(slot4)
  local slot5 = SlotRight
  local item5 = getInventoryItem(slot5)
  local slot6 = SlotFeet
  local item6 = getInventoryItem(slot6)
...
function checkDurability(text)
  if (not storage.titletwoEnabled) then
    return
end
  local durabilityPercentage = tonumber(text:match('Durability: (%d+)%%'))
  if durabilityPercentage and durabilityPercentage <= storage.durabilitySetText then
    playSound("/sounds/alarm.ogg")
    modules.game_textmessage.displayGameMessage('Low Durability!')
    g_window.setTitle(player:getName() .. " - Low Durability!")
  end
end
addTextEdit("durabilitySetText", storage.durabilitySetText or "70", function(widget, text)
  storage.durabilitySetText = tonumber(text)
end)

onTextMessage(function(mode, text)
  checkDurability(text)
end)
